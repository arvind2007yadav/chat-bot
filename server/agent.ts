...
async function retryWithBackoff<T>(
  ...
  maxRetries = 2           // Maximum number of retry attempts (default 3)
): Promise<T> {
  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      ...
      if (error.status === 429 && attempt < maxRetries) {
        ...
        console.log(`Rate limit over. Retrying in ${delay/1000} seconds....`)
  ...
  throw new Error("Max retries limit crossed") // This should never be reached
}

// Main function that creates and runs the AI agent
export async function callAgent(client: MongoClient, query: string, thread_id: string) {
  try {  
    // Validate incoming query to avoid passing undefined into LangChain
    if (typeof query !== 'string' || !query.trim()) {
      throw new Error('Invalid query provided to callAgent');
    }
    // Database configuration
    const dbName = "batiora"        // Name of the MongoDB database
    const db = client.db(dbName)              // Get database instance
    const collection = db.collection("items") // Get the 'items' collection
    console.log("callAgent invoked for thread:", thread_id, "db:", dbName)
    ...
    const itemLookupTool = tool(
        ...
        try {
          ...
          if (totalCount === 0) {
            ...
            return JSON.stringify({ 
              error: "No items found in Batiora", 
              message: "The Batiora database appears to be empty",
              count: 0 
            })
          }
          ...
          // Use embedding model from environment, defaulting to a stable v1beta model
          ...
          const vectorStore = new MongoDBAtlasVectorSearch(
            new GoogleGenerativeAIEmbeddings({
              ...
              model: embeddingModel,               // Embedding model (configurable)
            }),
            dbConfig
          )
          ...
          return JSON.stringify({ 
            error: "Failed to search Batiora database", 
      ...
      // Tool metadata and schema definition
      {
        ...
        description: "Gathers furniture item details from the Batiora database", // Description for the AI
    ...
    // Initialize the AI model (Google Generative AI)
    // Use chat model from environment, defaulting to a stable v1beta model
    const chatModel = process.env.GOOGLE_CHAT_MODEL || "gemini-2.5-flash";
    console.log("Initializing chat model:", chatModel);
...
Current time: {time}`,
        ...
        try {
          ...
          console.error('Model invoke failed:', invokeErr)
          throw new Error('Model invocation failed')
        }

       if (!result) {
  console.error('Model returned null/undefined:', result)
...
const hasToolCall =
  ...
  (result as any).tool_calls.length > 0

if (!hasText && !hasToolCall) {
  console.error('Invalid model response shape:', result)
  throw new Error('Model returned invalid response')
    ...
    // Debug: log the full final state returned by the workflow
    console.log("Final state from workflow:", JSON.stringify(finalState, null, 2))

    // Extract the final response from the conversation safely
    const lastIndex = finalState.messages.length - 1
    const lastMessage = finalState.messages[lastIndex]

    let response: string
    if (typeof lastMessage === 'string') {
      response = lastMessage
    } else if (lastMessage && typeof (lastMessage as any).content === 'string') {
      response = (lastMessage as any).content
    } else if (lastMessage && typeof (lastMessage as any).output === 'string') {
      response = (lastMessage as any).output
    } else {
      // Fallback: stringify whatever the model returned so the API doesn't crash
      console.warn('Unexpected message shape, stringifying lastMessage for response')
